 ...
 * 2.该类重新实现了QAbstractButton的nextCheckState()方法，方便通过check状态模拟按钮的"开关"状态，并
 * 可以配置在点击时自动/手动切换状态。
 * 3.该类重新实现了mousePressEvent()和mouseReleaseEvent()鼠标事件处理方法，添加了防抖和长按的功能
 * 处理
...
private:
    ...
    /*按钮长按*/
    bool longPressFlag;//长按使能标记 默认不使能
